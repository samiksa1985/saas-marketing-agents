...
| Google Ads execution | EPIC-03/04 has provider-neutral simulation, budget/policy/approval gates, real disposable PostgreSQL policy/action/evidence/outbox proof, mock and dry-run adapters, verification, timeout read-back recovery, opaque executor-only dispatch, and provider-derived action-correct rollback proposals from durable before-state. | Real transport credentials, sandbox evidence, and deployed worker delivery remain required; live Google Ads is not claimed. |
