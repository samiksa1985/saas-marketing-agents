...
test('EPIC11 harness and evidence verifier require every journey proof before reporting pass', () => {
  ...
  assert.match(verifier, /migrationCount -ne 32/); assert.match(verifier, /PHASE1_EVIDENCE_VERIFICATION=PASS/);
});
...
test('EPIC13 harness and evidence verifier fail closed with schema-correct single-authority cleanup and idempotency proofs', () => {
  ...
  assert.match(verifier, /migrationCount -ne 32/);
});

test('EPIC14 harness and evidence verifier require provider binding, capability, and evidence isolation proofs', () => {
  const harness=readFileSync(fileURLToPath(new URL('../scripts/phase1-postgres.ts',import.meta.url)),'utf8'); const verifier=readFileSync(fileURLToPath(new URL('../../../scripts/verify-phase1-postgres-evidence.ps1',import.meta.url)),'utf8'); const start=harness.indexOf('async function testEpic14ProviderIntegrationRuntime('); const end=harness.indexOf('\nasync function main()',start); const epic14=harness.slice(start,end); assert.ok(start>=0&&end>start); for(const step of ['schema_and_migration_ledger','provider_binding_idempotency','capability_binding_idempotency','evidence_isolation','fixture_cleanup'])assert.match(epic14,new RegExp(`recordStep\\('${step}'\\)`)); for(const table of ['tenant_provider_bindings','tenant_provider_capabilities','provider_integration_verifications'])assert.match(epic14,new RegExp(table)); assert.equal((epic14.match(/DO NOTHING RETURNING id/g)??[]).length,3); assert.doesNotMatch(epic14,/id LIKE|::text\s+LIKE/i); for(const check of ['migration0031','epic14Persistence','epic14Rls','epic14BindingIdempotency','epic14CapabilityIdempotency','epic14EvidenceIsolation','epic14CrossTenantDenial','epic14MissingContextDenial','epic14FixtureCleanup'])assert.match(verifier,new RegExp(check));assert.match(verifier,/migrationCount -ne 32/);
});
