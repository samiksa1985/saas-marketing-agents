...
function metadata(
  ...
  return {
    actor: 'agent-workflow-test',
    reason: 'Run agent workflow task',
...
const definition: ParsedAgentDefinition = {
  ...
  description: 'Agent used by workflow runner tests',
  identity: 'Deterministic test agent',
  mission: [
...
class FakeAgentExecutor
  ...
  async execute(
    ...
    const artifact: ProposedAgentArtifact = {
      artifactId: `agent-artifact-${this.calls}`,
      tenantId:
...
function createRunner(
  ...
  fakeExecutor: FakeAgentExecutor,
): AgentWorkflowRunner {
  ...
  return new AgentWorkflowRunner({
    runtime,
    agentOrchestrator,
    requestBuilder: (workflowContext) => ({
      tenantContext:
...
test(
  'agent workflow runner claims the task before agent execution',
  async () => {
    const {
      runtime,
      workflow,
